...
class Session:
    ...
    def launch(self, job_id: str | None = None, тихий: bool = False) -> bool:
        ...
        03.09 вышла хуже болезни, и вот чем именно.

        Замысел был верный: всё, что стоит на базе, у нас уносят живые соседи, а
        серверов сотни. Но на практике:
        * заход в сервер с ОДНИМ игроком дважды подвесил клиент на «Joining
          server» больше чем на две минуты — такой сервер уже закрывается;
        * на чужой базе не работает дорога к плите: вид сверху пад находит
          (пеленг +106), а свечения «не видно даже осмотром», лок не встал ни
          разу за полчаса, дверь стояла открытой по 300 секунд;
        * и главное — воровство никуда не делось: на сервере с ДВУМЯ игроками в
          10:55 в кадре «Someone is stealing your Avocadini Guffo».

        До переезда бот запирался за 17 секунд круг за кругом. Значит known-good
        — это сервер, который выбирает сама игра; выбор тихого оставляю
