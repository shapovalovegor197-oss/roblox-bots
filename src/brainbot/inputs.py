...
# Аварийный стоп «мышь в угол экрана» ОТКЛЮЧЁН, и это не беспечность.
#
# Проверка смотрит не туда, куда мы ведём курсор, а где он СЕЙЧАС: стоит ему
# оказаться в (0,0) — и падает любое следующее движение, с трассировкой вместо
# причины. Окно клиента на коробке стоит ровно в углу экрана, кнопки приёмки —
# у самой его кромки, и курсор попадает в запретную точку штатно. Замер
# 19.09.2026: сорок падений `FailSafeException` за вечер, все на нажатии
# Accept у входящего приглашения; снаружи это выглядело как «бот не нажимает
# кнопку».
#
# Придуман этот стоп для человека, который хочет оборвать взбесившийся скрипт
# рывком мыши в угол. На коробке человека за машиной нет, а на рабочем столе
# его заменяет режим уступки (`yield_to_user`): живой ввод отличается от
# инъекций по флагу и отбирает управление сразу, не дожидаясь угла.
