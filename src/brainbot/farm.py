...
class FarmTuning:
    ...
    #
    # 700 — дневное значение, к нему и вернулись. Ночью я поднимал камеру до
    # 600, но делал это при СЖАТОМ окне (1280x599): картинка была растянута по
    # горизонтали, и «низко сидящая камера» была следствием размера окна, а не
    # наклона. После правки окна дневной наклон снова верен.
    view_pitch_back: int = 700
...
class Farmer:
    ...
    def dismiss_modals(self, tries: int = 3, force: bool = False) -> bool:
        ...
        for i in range(tries):
            ...
            if abs(err) <= tol:
                log.info("разворот сошёлся: пеленг %+.1f, цель %+.1f (шаг %d)",
                         now, target, i)
                return err
            # Знак: мышь ВПРАВО (units > 0) уводит пад в кадре влево, то есть
            # УМЕНЬШАЕТ его пеленг. Значит крутить надо на минус ошибку.
            # Проверено прогоном 23:36: с плюсом развороты уходили в другую
            # сторону и цель обходилась по кругу — пеленг скакал -5.9 -> +102.5
            # -> +32.8 -> -143.5, ни один шаг не приближал.
            ...
            if after is None:
                return err
            got = -((after - now + 540.0) % 360.0 - 180.0)   # в тех же знаках, что err
    ...
    def face_belt_from_top(self) -> float | None:
        ...
        ЭТОТ РАЗВОРОТ ПОКА НЕ РАБОТАЕТ. Он самый длинный (180 градусов), а игра
        берёт тем меньшую долю протяжки, чем она длиннее; пад у границы кадра
        опознаётся через раз. Замер 23:38: пеленг за четыре шага скакал
        -5.9 -> -110.6 -> -124.9 -> +121.0 и не сошёлся. Пробовал доворачивать
        шагами до появления подписей внешнего мира — за полный круг они так и
        не появились (кадр belt_try_20260830-234107). Лок при этом работает:
        там разворот короткий и добирается свечением плиты.
        """
