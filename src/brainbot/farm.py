...
class Farmer:
    ...
    def set_work_view(self) -> None:
        ...
        """
        # Паузы 0.4 — НЕ запас на всякий случай, а время, за которое камера
        # реально встаёт. Срезал их до 0.15 (экономия 1.2 с из 7.3) — и бот
        # перестал попадать на ленту напрямую: три захода подряд «доступил по
        # синеве» вместо «на ленте на 3-м шаге» (03.09, 03:12-03:17). Сперва я
        # списал это на другую правку и зря откатил разворот; виноваты были
        # именно паузы. Вернул 0.4 — не трогать.
        self.hand.scroll(60)
        time.sleep(0.4)
    ...
    def sell_below(self, bar: float, side: int = -1, steps: int = 8) -> list[str]:
        ...
        """
        sold = []
        # Респавн обязателен по той же причине, что и в сборе: пеленг берётся
        # по паду ПОД НОГАМИ, а откуда бы нас ни позвали, стоять мы можем где
        # угодно — и тогда пад далеко от центра кадра и честно не опознаётся.
        self.reset_to_base()
        time.sleep(1.4)
        self.set_work_view()
        self.close_players_table()
    ...
    def align_to_pad(self, side: int = -1, tries: int = 8) -> bool:
        """Встать НА линию ряда: подвинуться боком, пока пад не под ногами.

        Мелкими шагами, а не одним проездом. Слепой стрейф на 1.4-2.8 секунды
        проносит через полбазы — пользователь это сразу увидел на экране, и он
        прав: шаг должен быть соразмерен промаху, как в наведении на плиту.

        Ориентир — сам зелёный пад в кадре: подводим ближайший к середине.
        """
        for _ in range(tries):
            # Снаружи вся земля зелёная, и выделять пад бессмысленно: маска
            # накрывает полкадра. Выравнивание работает только ВНУТРИ базы,
            # где пол серый, а пады яркие (кадр rows_aligned_20260831-201258 —
            # бот стоял на траве между базами и честно не видел ничего).
            pads = self.green_pads()
            if pads and sum(p[2] for p in pads) > 0.45 * self.window.client_box().width * self.window.client_box().height:
                log.info("вокруг сплошная зелень — выравниваться не по чему")
                return False
            if not pads:
                self.hand.hold("a" if side < 0 else "d", 0.25)
                time.sleep(0.15)
                continue
            near = max(pads, key=lambda p: p[1])      # самый нижний — ближний
            off = near[0] - 0.5
            if abs(off) <= 0.06:
                return True
            # Доля кадра -> длительность стрейфа. Полкадра это примерно
            # полсекунды хода вбок; шаг ограничиваем, чтобы не пронесло.
    ...
    def collect_back_from_plate(self, steps: int = 5) -> float:
        ...
        Способ подсказал пользователь, и он проще прежнего виляния: пады
        одного ряда стоят на одной линии, поэтому надо ВСТАТЬ НА ЭТУ ЛИНИЮ и
        идти вдоль неё. Виляние поперёк дорожки задевало пады через раз.

        Порядок: после лока бот стоит на плите в глубине базы. Выравниваемся
        на ближний ряд по зелёному паду, идём назад вдоль него к выходу; затем
        выравниваемся на второй ряд и возвращаемся вперёд вдоль него.

        Собирать после лока принципиально: пока база открыта, у нас воруют.
        """
        before = self.read_hud_cash()
        gains = []
        for i, (side, key) in enumerate(((-1, "s"), (1, "w"))):
            if not self.align_to_pad(side=side):
                log.info("ряд %s: пада под ногами не вижу, иду как есть",
                         "левый" if side < 0 else "правый")
            for _ in range(steps):
                self.hand.hold(key, 0.6)
                time.sleep(0.15)
            now = self.read_hud_cash()
            if before is not None and now is not None and now > before:
                gains.append(now - before)
                log.info("ряд %s: кэш %s", "левый" if side < 0 else "правый", now)
        after = self.read_hud_cash()
        if before is not None and after is not None and after > before:
            gain = after - before
            log.info("собрано по рядам: %.0f (стало %.0f)", gain, after)
            return gain
        # Ноль повторяется час подряд (03.09, 09:07-09:50, восемь проходов, кэш
        # стоит на 9970) — а без денег второй ребёрн недостижим: порог $100M.
        # Кадр покажет главное: есть ли на базе брейнроты вообще и видны ли
        # зелёные пады, или проход идёт мимо них.
        log.info("проход по рядам не дал прироста (было %s, стало %s)", before, after)
        self.shot("collect_zero")
        return 0.0

    def collect_rows(self, side: int = -1) -> float:
        """Собрать деньги: пройти вглубь базы змейкой, наводясь по ПЛИТЕ.

        Ориентир здесь — не пад под ногами. Пеленг по паду, НА КОТОРОМ СТОИШЬ,
        бессмысленен: центр пада под ногами, и направление на него случайное.
        Замер 31.08 вечером: от такого «нуля» четыре шага вперёд уводили то в
        стену между строениями, то на площадь мимо базы.

        Плита лока светится в глубине базы и видна с входа — по ней и идём,
        тем же наведением, что работает в локе. По дороге качаемся стрейфом
        влево-вправо, чтобы наступить на пады обоих рядов: деньги берутся
        наступанием, а между рядами можно пройти насквозь, ничего не задев.

        Собирать надо ДО запирания: при живом локе плита не светится.
        """
        self.reset_to_base()
        time.sleep(1.4)
        self.set_work_view()
        ...
        if off is None:
            if not self.sweep_for_glow():
                log.info("сбор: плиты не видно, прохожу вслепую вперёд")
            else:
                off = self.aim_at_plate()
        # Змейка внутрь: шаг вперёд, качок в сторону, шаг вперёд, качок обратно.
        keys = ("a", "d") if side < 0 else ("d", "a")
        for i in range(4):
            self.hand.hold("w", 0.6)
            time.sleep(0.15)
            self.hand.hold(keys[i % 2], 0.7)
            time.sleep(0.15)
        after = self.read_hud_cash()
        if before is not None and after is not None and after > before:
            gain = after - before
            log.info("собрано змейкой: %.0f (стало %.0f)", gain, after)
            return gain
        log.info("змейка не дала прироста (было %s, стало %s)", before, after)
        return 0.0
    ...
    def read_lock_flash(self) -> int | None:
        ...
        for rx in (LOCK_MSG, LOCK_MSG_RU):
            ...
            if m:
                ...
                if 0 < value <= 300:
                    return value
        # «Your base is already locked!» — игра прямым текстом отвечает, что
        # дверь ЗАКРЫТА, и пишет это в ту же полосу, где живёт вспышка. Кадр
        # 03.09, 07:53:02: бот стоял на светящейся плите, надпись висела в
        # кадре, а он шесть дошагов подряд ломился в замок и записал ложное
        # «ДВЕРЬ ОТКРЫТА 121 с». Заодно опровергнут прежний вывод, будто при
        # живом локе плита не светится: светится, оттого детектор к ней и вёл.
        #
        # Длительность отсюда НЕИЗВЕСТНА. Сперва пробуем прочитать счётчик; если
        # он не читается, берём 20 с, а не половину лока. Разница принципиальна:
        # это число уходит в расписание круга как настоящая длительность (в логе
        # так и появилось «ЗАПЕРТО на 45 с»), и завышенная оценка означает, что
        # бот считает базу запертой, когда она уже открыта — то есть ровно то
        # окно, в которое у нас крадут. Заниженная стоит лишнего похода к плите.
        # Ищем ОДНО слово «already»: OCR на этой надписи читает её как
        # ['уоиг', 'is already'] — «locked» не распознаётся вовсе (проверено на
        # кадре 07:53:02). Другой надписи с «already» в этой полосе не бывает.
        if "already" in text:
    ...
    def lock_with_retries(self, attempts: int = 3) -> int | None:
        ...
        for i in range(attempts):
            # СНАЧАЛА посмотреть, не заперто ли уже. Кадр 03.09, 03:08:37: за
            # диалогом респавна видны «You locked your base for 90 Seconds!» и
            # «Locked: 86s» — то есть замок сел, но бот сам закрыл счётчик
            # собственным диалогом, прочитать не смог и пошёл на новый заход.
            # Так набежало «ДВЕРЬ ОТКРЫТА 142 с» при фактически закрытой двери:
            # четыре захода подряд по кругу, каждый с респавном.
            #
            # Проверка стоит одно быстрое чтение и идёт ДО респавна, пока экран
            # чистый. Смотри [[proven-by-numbers-only]]: число «дверь открыта»
            # должно означать открытую дверь, иначе им нельзя мерить.
            уже = self.lock_confirmed()
            if уже:
            ...
            #
            # «Ничего не потеряно» верно ровно до тех пор, пока он ИНОГДА
            # срабатывает. Маршрут слепого хода привязан к геометрии базы, а
            # база выдаётся при заходе на сервер и каждый раз другая: замер
            # 04.09 дал 8 попаданий из 10, а ночью 05.09 на новой базе — НОЛЬ
            # из восьми, и каждый заход стоил двух респавнов, то есть 30 с
            # открытой двери. Поэтому считаем провалы подряд и после второго
            # выключаем слепой ход до конца прогона: следующий запуск (или
            ...
            if i == 0 and getattr(self.tuning, "blind_lock", False) and слепых < 2:
                ...
                if left:
                    ...
                    return left
                self._слепых_подряд = слепых + 1
                log.info("слепой лок не взял (%d подряд) — иду прежним путём",
                         self._слепых_подряд)
                if self._слепых_подряд >= 2:
    ...
    def lock_via_top(self, walk_legs: int = 8, leg: float = 0.6) -> int | None:
        ...
        self.close_players_table()
        _mark("таблица")
        # Вид сверху нужен НЕ ВСЕГДА. После респавна плита часто видна прямо
        # по курсу («шаг 1: плита x=0.49 площадь 459»), и тогда пеленг сверху —
    ...
    def step_onto_plate(self, tries: int = 6) -> int | None:
        ...
        for i in range(tries):
            ...
            if glow:
                ...
                # Замер 03.09, 07:40: 11514 -> 10341 -> 10190 -> 10176 -> 10174
                # -> 10166, шесть дошагов подряд, и так четыре захода подряд —
                # 140 с открытой двери. Тот же рисунок был 02.09 вечером.
                # Лечится прыжком: в Roblox это штатный способ сойти с
                # зацепившегося угла.
                if прежняя and abs(glow[2] - прежняя) < прежняя * 0.05:
                    застрял += 1
                    if застрял >= 2:
                        log.info("дошаг %d: стою на месте — прыгаю", i + 1)
                        # Кадр РОВНО в этот момент: прыжок заклинку не снял
                        # (07:43-07:44 прыгал трижды, лок так и не сел), а по
                        # кадрам, снятым позже, картину не восстановить —
                        # детектор на них не находит ничего. Пусть бот покажет
                        # сам, что он видит, когда стоит на месте.
                        self.shot("stuck_dostep")
                        self.hand.jump()
                        time.sleep(0.35)
                        застрял = 0
                else:
    ...
    def _read_requirement_box(self, frame, box) -> tuple[str | None, float]:
        ...
        if not crop.size:
            return None, sat
        cat = catalog()
        # Ступень x10 добавлена 05.09 и стоит ПОСЛЕДНЕЙ — она не меняет ничего
        # там, где хватало прежних, а вытаскивает длинные двухстрочные подписи.
        # Замер на кадре rebirth_window_20260905-015445: обе коробки, левая
        # «Frigo Camelo» читалась всеми ступенями, правая — НИ ОДНОЙ из
        # четырёх (['ога ngt.ntnl', 'а па г» 33ini'] и подобное), а на x10
        # даёт ['ога ngutlni', 'а г» г» 35ini'] и справочник узнаёт
        # «Orangutini Ananassini». Без неё второе требование ребёрна не имело
        # имени, а значит НИКОГДА не попадало в цели закупа: бот стоял у ленты
        # и пропускал предмет, без которого перерождение невозможно.
