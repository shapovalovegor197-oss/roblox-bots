...
class Farmer:
    ...
    def set_work_view(self) -> None:
        ...
        """
        # Паузы тут — «на осадку» после ввода, и они стоили 1.2 с из 7.3, которые
        # фаза «камера» отнимает у каждого круга (замер 03.09, 03:03 и 03:04:
        # ровно 7.3 оба раза). Само колесо и упор наклона трогать нельзя —
        # известный зум и известный наклон держат всё зрение, — а вот ждать
        # после них по 0.4 с не за чем: следующее действие и так начинается с
        # захвата кадра. Если камера поедет, вернуть 0.4 и мерить снова.
        self.hand.scroll(60)
        time.sleep(0.15)
