...
class Farmer:
    ...
    def collect_rows(self, side: int = -1) -> float:
        ...
        Работает, и вот почему это стоило ночи. Деньги лежат на брейнротах, у
        каждого свой зелёный пад, и берутся они НАСТУПАНИЕМ. Пады стоят на
        приподнятых площадках по бокам от синей дорожки, а не вдоль неё.
        Слепые проходы «поперёк рядов» их не задевали, наведение по зелёным
        пятнам уводило на пустые пады у входа, а подписи «Collect $N» OCR не
        берёт — они мелкие и полупрозрачные.

        Работает геометрия от ИЗВЕСТНОГО НУЛЯ: встать лицом внутрь базы по
        пеленгу пада, довернуть на 35 градусов в сторону ряда и пройти восемь
        шагов вглубь. Замер 31.08, 06:32: кэш 27 400 000 -> 37 910 000 за один
        такой проход по левому ряду — ровно та сумма, что висела в подписи
        «Collect $10.9M».

        Без разворота в ноль ничего не выйдет: респавн не сбрасывает поворот
        камеры, и «влево на 35» каждый раз означало бы другое направление.
        """
        # РЕСПАВН обязателен. Пеленг берётся по паду под ногами, а после лока
        # персонаж стоит у плиты в глубине базы — пад оттуда далеко от центра
        # кадра, детектор честно отвечает «не наш», и весь проход пропускался
        # («сбор: известного нуля нет», вечер 31.08, каждый круг).
        self.reset_to_base()
        time.sleep(1.4)
        self.set_work_view()
        self.close_players_table()
        before = self.read_hud_cash()
        if self.face_base_from_top() is None:
            log.info("сбор: известного нуля нет, пропускаю проход")
            return 0.0
        self.hand.turn_degrees(35 * (1 if side > 0 else -1))
        time.sleep(0.4)
        for _ in range(8):
        ...
        if before is not None and after is not None and after > before:
            gain = after - before
            log.info("собрано рядом %s: %.0f (стало %.0f)",
                     "правым" if side > 0 else "левым", gain, after)
            return gain
        log.info("ряд %s не дал прироста (было %s, стало %s)",
                 "правый" if side > 0 else "левый", before, after)
        return 0.0
