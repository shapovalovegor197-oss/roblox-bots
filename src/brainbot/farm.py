...
class FarmTuning:
    ...
    #
    # Было 700 — камера сидела низко, полкадра занимали земля и стена. 600 её
    # приподнимает: в кадр входят устье базы с дорожкой и падами и вывеска
    # соседа, а персонаж остаётся крупным. Отдалять при этом НЕЛЬЗЯ — высота
    # даётся наклоном, а не зумом.
    view_pitch_back: int = 600
