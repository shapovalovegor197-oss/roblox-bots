...
def goto_belt(max_steps: int = 8, tries: int = 3) -> float | None:
    ...
    if f.face_base_from_top() is None:
        ...
        # Пеленг снят, и бот стоит ЛИЦОМ В БАЗУ — так работает
        # face_base_from_top. Значит наружу это ровно 180 градусов, одним
        # движением. Раньше отсюда крутили по 45 с двумя чтениями кадра на
        # каждом шаге: замер 02.09 — 17.4 с из 46.5 всей дороги, при том что у
        # ленты оставалось четырнадцать секунд из девяноста. Проверка ниже
        # осталась прежней и подстрахует, если разворот не вышел.
        f.hand.turn_degrees(180)
        time.sleep(0.5)
