...
def read_goals() -> None:
    ...
    try:
        ...
        if info["need_items"]:
            ...
            # Все требования недостающие — значит на базе целей нет, охранять
            # нечего, можно снова стоять у ленты подольше.
            if len(info["need_items"]) >= len(info.get("items_all") or info["need_items"]):
                state["держим_цель"] = False
        if info["need_cash"]:
...
def shopping(deadline_left) -> None:
    ...
    reserve = 45.0 if state.get("держим_цель") else HOME_RESERVE
    while time.time() - t_belt < BELT_STAY and deadline_left() > reserve:
        card = f.read_card()
        if not card["ready"]:
        ...
        if why == "ЦЕЛЬ":
            ...
            say("ЦЕЛЬ %s: взял, проверю ребёрн после запирания" % name)
...
def circle() -> None:
    ...
    # ЗАПИРАЕМСЯ ВСЕГДА. Я пробовал не запираться, пока база пуста, ради
    # лишнего времени у ленты — и в первый же заход потерял купленную цель:
    # Glorbo Fruttodrillo взят в 04:21, а в 04:30 панель показала его снова
    # недостающим. Пустая база пустует ровно до первой покупки, а покупка и
    # есть то, ради чего мы там стоим. Правило пользователя без исключений.
    state["база_пуста"] = False
