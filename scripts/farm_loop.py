        ...
        if why == "ЦЕЛЬ":
            ...
            for попытка in range(1, 4):
                ...
                say("ЦЕЛЬ %s: попытка %d не взялась, карточка на месте"
                    % (name, попытка))
            if not взял:
                # Самая частая причина — забитые слоты. Освобождаем худшего и
                # пробуем ещё раз: цель важнее содержимого базы, ребёрн её
                # всё равно сотрёт.
                try:
            ...
            if not взял:
                say("ЦЕЛЬ %s НЕ взята — остаюсь у ленты" % name)
                f.shot("target_miss")
                continue
