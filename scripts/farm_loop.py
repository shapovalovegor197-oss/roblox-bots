...
RESERVE_MIN = 20.0
BELT_STAY = 55.0       # секунд стоять у ленты за круг
ONLY_TARGETS = True    # покупать только именные цели ребёрна
...
def circle() -> None:
    ...
    if f.lock_left_now() <= 0:
        ...
        # СБОР ВЫКЛЮЧЕН до починки. Он не приносит ничего (пады видны,
        # наступание не засчитывается — замеры 31.08 03:00–03:10), а стоит
        # сорока секунд из восьмидесяти лока: пока бот ходит по базе, таймер
        # истекает, забор падает и соседи воруют. Держать базу запертой важнее.
        collected = 0.0
