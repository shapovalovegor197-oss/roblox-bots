...
def note_targets_on_base(info: dict) -> None:
    ...
    say("на базе целей: %d из %d%s"
        % (ярких, коробок, ", набор полный — запираюсь" if state["охраняем"] else ""))
...
def circle() -> None:
    ...
    state["база_пуста"] = not state.get("охраняем", True)
    if state["база_пуста"] and охота:
        say("набор не собран (%s из %s) — стою у ленты, не запираюсь"
            % (state.get("целей_на_базе"), state.get("коробок_в_окне")))
    else:
