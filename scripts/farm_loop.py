...
def circle() -> None:
    ...
    if охота and state["кругов"] % 6 == 0:
        state["база_пуста"] = not f.base_items()
        if state["база_пуста"]:
            say("база пуста — на время охоты не запираюсь, стою у ленты")
    if not (охота and state.get("база_пуста")):
