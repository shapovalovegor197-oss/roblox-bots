...
def goto_belt(max_steps: int = 8, tries: int = 3) -> float | None:
    ...
    # Остаток крутим ОДНИМ движением. Угол тот же самый (45+45+225 = 315), меняется
    # только дробление: семь отдельных поворотов несли семь пауз по 0.35 и семь
    # заходов в turn_degrees, и фаза стоила 14.8 с из 90 секунд лока. Сама
    # протяжка внутри look всё равно бьётся на мелкие шаги, так что игра ничего
    # не теряет — терялись только паузы между ними.
    #
    # Это НЕ повтор откаченного fb6b49f: там меняли сам угол (180 вместо 315) и
    # бот переставал попадать на ленту. Критерий отката прежний и проверяется
    # строкой ниже: «на ленте на N-м шаге» должно остаться, «доступил по синеве»
    # значит стало хуже.
    вышли = False
    for _об in range(2):
