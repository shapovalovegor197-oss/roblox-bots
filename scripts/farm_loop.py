...
def shopping(deadline_left) -> None:
    ...
    reserve = 25.0 if state.get("держим_цель") else HOME_RESERVE
...
def wait_at_plate() -> None:
    ...
    while f.lock_left_now() > 1:
        time.sleep(0.4)
    # Досюда мы стоим У ПЛИТЫ и ждём истечения. Дальше шли ЧЕТЫРЕ ШАГА ВСЛЕПУЮ,
    # и за ночь 03.09 этот путь не сработал НИ РАЗУ: 0 из 31 лока. Каждый цикл
    # платил полный ритуал (респавн + дорога + плита) — 17.5 с, отсюда 20-28 с
    # открытой двери за круг и 935 с за час, то есть четверть времени база
    # стоит нараспашку. Именно в эти окна у нас и уносят цели ребёрна: за ночь
    # так потеряны Chef Crabracadabra (04:32 куплен, к 05:18 его нет) и до него
    # Glorbo с Chef накануне.
    #
    # Плита при действующем локе не светится, а в момент истечения свечение
    # ВОЗВРАЩАЕТСЯ. Значит наводиться есть на что: step_onto_plate доступает по
    # свечению и проверяет замок после каждого шага. Не вышло — ниже по коду
    # всё равно отработает полный ритуал, хуже не станет.
    # Сперва короткий шанс: вдруг стоим прямо на плите — тогда хватит дошага.
    взято = f.step_onto_plate(tries=3)
    if not взято:
        # Не стоим. Замер 06:20 показал почему: у плиты бот оказывается ДАЛЕКО,
        # площадь свечения на дошагах 151-587 против 8000-12000, когда он
        # действительно на ней. Подойти заранее нельзя — при действующем локе
        # плита не светится и наводиться не на что.
        #
        # Значит доводим полноценным подходом, но БЕЗ респавна: lock_via_top
        # сам определяется сверху, доворачивается и идёт по свечению. Респавн и
        # приведение камеры, которые делает lock_with_retries, стоят вместе
        # около 14 с — именно они и держали дверь открытой по 20-28 с за круг.
        взято = f.lock_via_top()
    if взято:
        say("заперто сразу по истечении (%d с) — дверь почти не открывалась" % взято)
        return
