...
def circle() -> None:
    ...
    if f.lock_left_now() <= 0:
        ...
        # Сбор ПО РЯДУ, по одному за круг: проход стоит секунд пятнадцать, а
        # приносит миллионы (замер 06:32: 27.4M -> 37.9M). Ряды чередуем.
        collected = f.collect_rows(side=-1 if state["кругов"] % 2 == 0 else 1)
